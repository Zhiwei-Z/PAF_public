...
if __name__ == "__main__":
    lr_s = [0.001]
    scaleS = [0.01, 0.03, 0.05, 0.1, 0.3]
    seeds = list(range(4))
    gpu = 0
    commands = []
    num_process = 7
    for env_name in ["ant"]:  # ["ant", "hc", "hopper", "humanoid", "walker"]
        for lr in lr_s:
            ...
            for scale in scaleS:
                for s in seeds:
                    commands.append(assemble_command(env_name=env_name,
                                                     ...
                                                     supervision_mode='gv',
                                                     epsilon_s=0.05,
                                                     train_separate_params=True,
                                                     gradient_loss_scale=scale,
                                                     use_relu=True,
                                                     exp_name=assemble_exp_name(env_name=env_name, var_name="lr+scale", var_value='{}+{}'.format(str(lr), str(scale)), dagger='True', seed=s),
                                                     data_path='../../BC_ant_tune_scale'))
                    gpu += 1
    i = 0
