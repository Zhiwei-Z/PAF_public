...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0 and load_initial_expertdata is not None:
            # print("initial iteration, directly load trajectories from ", load_initial_expertdata)
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, gradient_batch = self.agent.sample(self.params['train_batch_size'])

            # TODO use the sampled data to train an agent
            # HINT: use the agent's train function
            # HINT: keep the agent's training log for debugging
            train_log = self.agent.train(ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch, gradients=gradient_batch)
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        while count < len(paths):
            paths[count]["action"] = expert_policy.get_action(paths[count]["observation"])
            count += 1
        return paths
